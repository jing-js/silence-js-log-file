...
class FileLogger {
  ...
  access(method, code, duration, bytesRead, bytesWritten, user, ip, userAgent, url) {
    ...
    this.accessWriter.write((cluster.isWorker ? `[${cluster.worker.id}] ` : '') + `[${util.formatDate()}] [${code !== 0 && code < 1000 ? code : 200}] [${method}] [${ds}] [${bytesRead}] [${bytesWritten}] [${user ? user : '-'}] [${ip}] "${userAgent || ''}" ${url}\n`);
  }
  _write(level, ...args) {
    if (args.length === 0) {
      return;
